...
namespace G1ANT.Addon.Selenium
{
    public class SeleniumWrapper
    {
        ...
        public string GetAttributeValue(string attributeName, string elementXPath, SeleniumIFrameArguments search, TimeSpan timeout)
        {
            PreCheckCurrentWindowHandle();
            if (!string.IsNullOrEmpty(search.IFrameSearch?.Value))
                webDriver.SwitchTo().Frame(FindElement(search.IFrameSearch.Value, search.IFrameBy.Value, timeout));
            var element = FindElement(elementXPath, "xpath", timeout);
        ...
        {
            return GetAttributeValue("outerHTML", "//html", search, timeout);
        }

        public string GetInnerHtml(SeleniumCommandArguments search, TimeSpan timeout)
        {
            return GetAttributeValue("innerHTML", search, timeout);
        }

        public string GetOuterHtml(SeleniumCommandArguments search, TimeSpan timeout)
        {
            return GetAttributeValue("outerHTML", search, timeout);
        }
